# -*- coding: cp1252 -*-
from Tkinter import *
from MouseWheel import ScrollingArea
...
class MultiListbox(PanedWindow):
    def __init__(self,master,lists):
        ...
        self.lists = []
        self.columns=[]
        self.sb = Scrollbar(master, orient=VERTICAL, command=self._scroll,borderwidth=1)
        for l,w in lists:
            self.columns.append(l)
            frame = Frame(self); frame.pack(side=LEFT, expand=0, fill=BOTH)
            tl=Button(frame, text=l, borderwidth=2, relief=RAISED)
            tl.pack(fill=X)
            ...
            lb.bind('<Enter>',lambda e,s=self:s.focus_force())
        ...
        self.sb.pack(fill=Y,side=RIGHT,expand=NO)

        for l in self.lists:
            l['yscrollcommand']=self.sb.set
...
#        self.bind('<Enter>',lambda e,s=self:s.focus_force())         ACHO QUE N SERA PRECISO
	...
	def wheel_event(self, evt): 
		...
		lines = evt.delta // 120 
		self.lb.yview_scroll(-lines, UNITS) 

    def _b2motion(self, x, y):
        for l in self.lists: l.scan_dragto(x, y)
        return 'break'

    def _scroll2(self, evt):
        for l in self.lists:
            lines = evt.delta // 120 
            l.yview_scroll(-lines,UNITS)
            #apply(l.yview_scroll, args)
...
if __name__ == '__main__':
    ...
    mlb.pack(expand=0,fill=Y,side=TOP)
    tk.mainloop()